        ...
        for (var i = 0; i < questKeys.length; i++) {
            ...
            if (questKeys[i] == 'hsForm') {
                ...
                for (var j = 0; j < questJson[questKeys[i]].length; j++) {
                    ...
                    // opts += `<p class="tag">${questKeys[i][0] + (j + 1)}:${quest.q1}</p>
                    //     <div class="quiz_sct checkRadioContainer">
                    //     <span class="optiont">Never</span><input type="radio" name="${questKeys[i] + j}" value="1">
                    //     <span class="optiont">Sometimes</span><input type="radio" name="${questKeys[i] + j}" value="2">
                    //     <span class="optiont">Frequently</span><input type="radio" name="${questKeys[i] + j}" value="3">
                    //     <span class="optiont">Very Frequently</span><input type="radio" name="${questKeys[i] + j}" value="4">
                    //     <span class="optiont">Always</span><input type="radio" name="${questKeys[i] + j}" value="5">
                    //     </div>`;

                    opts += `<p class="tag">${questKeys[i][0] + (j + 1)}:${quest.q1}</p>
                        <div class="quiz_sct checkRadioContainer">
                            <label><input type="radio" name="${questKeys[i] + j}" value="1"><span onclick="removeErrMsg()">Never</span></label>
                            <label><input type="radio" name="${questKeys[i] + j}" value="2"><span onclick="removeErrMsg()">Sometimes</span></label>
                            <label><input type="radio" name="${questKeys[i] + j}" value="3"><span onclick="removeErrMsg()">Frequently</span></label>
                            <label><input type="radio" name="${questKeys[i] + j}" value="4"><span onclick="removeErrMsg()">Very Frequently</span></label>
                            <label><input type="radio" name="${questKeys[i] + j}" value="5"><span onclick="removeErrMsg()">Always</span></label>
                        </div>`;
                }
            ...
            if (i == currentTab && questKeys[i] != 'hsForm') {
                ...
                for (var j = 0; j < questJson[questKeys[i]].length; j++) {
                    ...
                    if (v) {
                        ...
                        document.querySelector(`.errmsg`).innerHTML = "Please select answer for question " + questKeys[i][0]+(j + 1);
                        document.querySelector(`.errmsg`).style.display = "block";
                        return false;
                    };
                }
                questAvg = questAvg / 3;
                avgJson[questKeys[i]].avg = questAvg.toFixed(2);
                avgJson["INCLUSION SUPERFACTOR"].ans.push(questAvg);

                var totolAvg = 0;
                for (var j = 0; j < avgJson["INCLUSION SUPERFACTOR"].ans.length; j++) {
                    totolAvg += avgJson["INCLUSION SUPERFACTOR"].ans[j];
                }
                totolAvg = totolAvg / avgJson["INCLUSION SUPERFACTOR"].ans.length;
                avgJson["INCLUSION SUPERFACTOR"].avg = totolAvg.toFixed(2);
            }
        }
                    ...
                    graphJson.push(
                        {
                            ...
                            normal: {
                                fill: (avgJson[k].avg - 4 < 0 ? 'red' : '#64b5f6'),
                                stroke: null,
                                label: { enabled: true }
                            }
                        }
